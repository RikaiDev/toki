...
pub async fn handle_plane_command(action: PlaneAction) -> Result<()> {
    ...
    match action {
        PlaneAction::Test => handle_test(&client).await,
        PlaneAction::Projects => handle_projects(&client).await,
        PlaneAction::Issues { project, search } => handle_issues(&client, project, search).await,
        PlaneAction::MyIssues => handle_my_issues(&client).await,
    }
}

async fn handle_test(client: &PlaneClient) -> Result<()> {
...
async fn handle_projects(client: &PlaneClient) -> Result<()> {
...
async fn handle_issues(
    ...
    if let Some(query) = search {
        handle_search_issues(client, &query).await
    } else if let Some(project_id) = project {
        handle_project_issues(client, &project_id).await
    } else {
...
async fn handle_search_issues(client: &PlaneClient, query: &str) -> Result<()> {
...
async fn handle_project_issues(client: &PlaneClient, project_id: &str) -> Result<()> {
...
async fn handle_my_issues(client: &PlaneClient) -> Result<()> {
